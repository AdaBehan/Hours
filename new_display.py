...
# Own Imports
from back.application_mgmt import View_control 
...
class new_display():
    ...
    def __init__(self):
        """ Start up function call 
            references created 
                self.glade           = Gtk.Builder()
                self.edit_dialog     = edit dialog 
                self.color_dialoug   = color dialog 
                self.ignore_dialouge = ignore dialog

                self.sch             = scheduler module
                self.view_control    = view_control class in application_mgmt
        """
...
#Main Stuff 
    ...
    def set_active(self):
        ...
        earlyest_time = False
        name_list = []
        current_time = iter_time.next() #keeps track of the time
        
        
        while current_time != time_now:
            block_counter += 1
            current_time = iter_time.next() #increments to next time stamp

            if earlyest_time == False :  #gets the earliest entry from all_active
                if len(all_active) == 0:
                    earlyest_time = [0]
                else:
        ...
        # all entry's are now in the future
        while current_time < end_of_day: 
            block_counter += 1
            current_time = iter_time.next() 
...
# Ignore List stuff [Finished]
    ...
    def unignore_pressed(self,button,linked_name):
        """ When user press the unigore button send request to remove that 
            app from the ignore list
        """
        self.view_control.remove_ignore(linked_name)


    def ignore_dialog_close(self,arg):
        """ Closes the ignore dialog
        """
